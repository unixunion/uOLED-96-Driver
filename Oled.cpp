...
uint8_t OLED::DrawText(uint8_t column, uint8_t row, uint8_t font_size, char *mytext, unsigned int color)
{
	...
	{
		Serial.write(mytext[i]); 			// character to write
	}
	
	Serial.write(0x00); 				// string terminator (always 0x00)
...
uint8_t OLED::DisplayBitChar(uint8_t slot, uint8_t x, uint8_t y, unsigned int color)
{
	...
	Serial.write(OLED_DISPLAY_CHAR);
...
uint8_t OLED::DrawButton(bool state, uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t font_size, uint8_t button_colour, uint8_t text_colour, char* text)
{
	...
	Serial.write(button_colour >> 8);				// MSB			
	Serial.write(button_colour & 0xFF);				// LSB

	Serial.write(font_size);

	Serial.write(text_colour >> 8);					// MSB			
	Serial.write(text_colour & 0xFF);				// LSB
	...
	{
		Serial.write(text[i]); 			// character to write
	}
